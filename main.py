...
def fetch_alpha_vantage_earnings(ticker):
    ...
    logger.info(f"Step 1: Fetched per-symbol CSV data for {ticker}: {csv_data!r}")
    ...
    if csv_data:
        ...
        try:
            ...
            logger.error(f"Step 15: Error parsing per-symbol CSV for {ticker}: {e}")
    ...
    if full_csv_cache and full_csv_timestamp and (current_time - full_csv_timestamp).total_seconds() < CACHE_DURATION:
        ...
        csv_data = fetch_alpha_vantage_data("EARNINGS_CALENDAR", params)
        logger.info(f"Step 18: Fetched full earnings calendar CSV: {len(csv_data) if csv_data else 0} bytes")
        if csv_data:
            full_csv_cache = csv_data
            full_csv_timestamp = current_time
        else:
            logger.error(f"Step 19: Failed to fetch full earnings calendar CSV")
            return "N/A"
        ...
        for row in csv_reader:
            if row.get("symbol") == ticker:
                ...
                if report_datetime > current_time:
                    ...
                    return "N/A"
        logger.warning(f"Step 30: No upcoming earnings date found for {ticker} in full CSV")
        return "N/A"
    except Exception as e:
        logger.error(f"Step 31: Error parsing full earnings calendar CSV for {ticker}: {e}")
        return "N/A"
...
def get_latest_quote(ticker):
    ...
    if data and "Global Quote" in data:
        ...
        return {
            "price": parse_float(quote.get("05. price", "N/A")),
            "volume": parse_float(quote.get("06. volume", "N/A"))
        }
...
async def scrape_ticker(ticker: str):
    ...
    volume_30d_avg = quote_data["volume"]
    ...
    if ticker != "SPY":
        ...
        shares = overview_data["shares"]
        if volume_30d_avg != "N/A" and shares != "N/A" and shares != 0:
            liquidity_ratio = round(volume_30d_avg / shares, 4)
    ...
    days_to_cover = "N/A"
    if short_interest != "N/A" and volume_30d_avg != "N/A" and volume_30d_avg != 0:
        days_to_cover = round(short_interest / volume_30d_avg, 2)
    ...
    # Include market_cap and liquidity_ratio only for non-ETFs
    if ticker != "SPY":
        result["market_cap"] = market_cap
        result["liquidity_ratio"] = str(liquidity_ratio)
